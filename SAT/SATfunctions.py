...
def pad(l: T_NumberAsBoolList, length: int) -> T_NumberAsBoolList:                           #adds a padding in the number as boolean
    assert length > 0 and length >= len(l), '\nl:\t{}\nlength:\t{}'.format(l, length)

    return [False for _ in range(length - len(l))] + l

def bool2int(l: T_NumberAsBoolList) -> int:                                                 #bool to int (example, [F,T,F,T]->5)
    result = 0
    l_b = []
...
def int2boolList(n: int) -> List[bool]:                                                     #int to bool (example, 5->[T;F;T])
    result = []
    base2 = format(n, "b")
    for bit in base2:
        if bit == "1":
...
def all_F(l: T_NumberAsBoolList) -> T_Z3Clause:                                           #all False
    return And([Not(b) for b in l])

def at_least_one_T(bools: List[Bool]) -> T_Z3Clause:                                      #al least 1 (min 1 T)
    return Or(bools)

def at_most_one_T(bools: List[Bool]) -> T_Z3Clause:                                       #at most 1  (max 1 T)
    ### HEULE ENCODING
    l = len(bools)
    result = []
    if l <= 3:  ###base case
...
def exactly_one_T(bools: List[Bool]) -> T_Z3Clause:                                      # 1 T
...
def ne(l1, l2) -> T_Z3Clause:                                                            #not equal
...
def eq(l1: T_Number, l2: T_Number) -> T_Z3Clause:                                        #equal
    l1, l2 = get_bool_lists(l1, l2)
...
def __gte_same_len(l1: T_NumberAsBoolList, l2: T_NumberAsBoolList) -> T_Z3Clause:      #l1>=l2 with same len
...
def gte(l1: T_Number, l2: T_Number) -> T_Z3Clause:                                            #l1>=l2
...
def __gt_same_len(l1: T_NumberAsBoolList, l2: T_NumberAsBoolList) -> T_Z3Clause:        #l1>l2 same len
    ### AND-CSE Encoding: Common SubExpression Elimination
    if len(l1) == 1:
...
def gt(l1: T_Number, l2: T_Number) -> T_Z3Clause:                                      #l1>l2
...
def lte(l1: T_Number, l2: T_Number) -> T_Z3Clause:                                  #l1<=l2                             
    return gte(l1=l2, l2=l1)

def lt(l1: T_Number, l2: T_Number) -> T_Z3Clause:                                   #l1<l2
    return gt(l1=l2, l2=l1)

def sum_b(l1: T_Number, l2: T_Number) -> T_Z3Clause:                                #binary sum (into a Z3 formula)
    l1, l2 = get_bool_lists(l1, l2)
    max_len = max(len(l1), len(l2))
    l1 = pad(l1, max_len)
    l2 = pad(l2, max_len)
